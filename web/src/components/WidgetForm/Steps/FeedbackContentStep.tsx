...
interface FeedBackContentStepProps {
    feedbackTypeChosen: FeedbackType;
    onFeedbackRestart: () => void;
    onFeedbackSent: () => void;
}

export function FeedbackContentStep({ feedbackTypeChosen, onFeedbackRestart, onFeedbackSent }: FeedBackContentStepProps) {
    const [screenshot, setScreenshot] = useState<string | null>(null);
    const [comment, setComment] = useState(''); // string na textarea
    
    const feedbackTypeInfo = feedbackTypes[feedbackTypeChosen];
    ...
    return (
        <>
            <header>
                <button 
                    type="button" 
                    className="absolute top-5 left-5 text-zinc-500 hover:text-zinc-100"
